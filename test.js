...
test('should return a url with a valid token and query string', () => {
  ...
  expect(result).toEqual("https://api.urlbox.io/v1/" + myapikey + "/" + token + "/png?" + query);
});
...
test('should error if url is wrong type', () => {
  let options = {url: 2};
  expect(() => urlbox.buildUrl(options)).toThrow("url should be of type string (something like www.google.com)");
});
test('cookies', () => {
  const options = {
    url: 'bbc.co.uk',
